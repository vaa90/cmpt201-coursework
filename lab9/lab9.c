...
data from? How can you tell in the code?
    stdin, from the read intruction
...
void *handle_client(void *arg) {
  ...
  printf("New client created! ID %d on socket FD %d", client_id, cfd);
  free(client);
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE - 1)) > 0) {
    ...
    total_message_count++;
    int msg_count = total_message_count;
    pthread_mutex_unlock(&count_mutex);

    printf("Msg #   %d; Client ID %d: %s", msg_count, client_id, buf);
  }
  ...
  printf("Ending thread for client %d", client_id);
  close(cfd);
