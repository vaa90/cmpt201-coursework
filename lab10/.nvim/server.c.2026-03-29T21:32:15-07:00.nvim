...
- pointer to struct acceptor_args which stores data for thread operations
...
- threads are used to read client messages
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
  ...
  while (aargs->run) {
    if (num_clients < MAX_CLIENTS) {
      ...
      if (cfd == -1) {
        ...
        pthread_create(&threads[num_clients - 1], NULL, run_client, &client_args[num_clients - 1]);
      }
    }
  }
